...
const ConsultationForm = () => {
  const componentRef = useRef();
    ...
    const [formData, setFormData] = useState({
      ...
      const handleSubmit = (e) => {
        ...
        const requiredFields = ['firstName', 'lastName', 'age', 'gender', 'dob', 'consultingDoctorName', 'obervation', 'consultantCharge', 'clinicCharge'];
  ...
  />
          ...
          <div className="col-md-6">
          <label htmlFor="doctorName" className="form-label"><b>Observation</b></label>
            <input
              type="text"
              className="form-control"
              id="obervation"
        ...
        <div className="row mt-3 mb-4">
          <div className="col-md-12 text-end">
            <button type="submit"  className="btn  me-2" onClick={handleCancel}  style={{backgroundColor:'teal', color:'white'}}>Cancel</button>
            <button type="button" style={{ backgroundColor: 'teal', color: 'white' }} className="btn " onClick={handleSubmit} >Submit</button>
          </div>
        </div>
        </div>
        {showAlert && <FloatingAlert message="Please fill in all required fields before submitting." />}
      </form>
      </>
      )}
...
{!showForm && submittedData && (
    ...
    <div className='d-flex justify-content-end align-items-end'> 
     <ReactToPrint
              ...
              />
        <button type="button" className="btn me-2 ms-2" style={{ backgroundColor: 'green', color: 'white',  }} onClick={handleCancel}>
              Go to Previouspage
            </button>
        </div>

        <div className="mt-10" style={{marginLeft:'100px', marginTop:'50px', width:'65%',
        height: '600px',
        ...
        fontFamily: 'serif' }}>
          ...
          <h3> <b>Doctor Consultation Form</b></h3>
          <p>OP Number: {submittedData.opNumber}</p>
          <p>Patient Name: {submittedData.firstName} {submittedData.lastName || ''}</p>
          <p>Gender: {submittedData.gender}</p>
          <p>Age: {submittedData.age}</p>
          <p>Date of Birth: {submittedData.dob}</p>
          <p>Consulting Doctor Name: {submittedData.consultingDoctorName}</p>
